...
const AdminMap = () => {
  ...
  const fetchEstablishments = useCallback(async () => {
    try {
      ...
      const [{ data: applicationsData, error: applicationsError }, 
             { data: establishmentsData, error: establishmentsError }, 
  ...
  return (
    <DashboardLayout title="Map" userRole="admin">
      <div className="flex flex-col h-[calc(100vh-4rem)] space-y-4">
        <div className="flex flex-col space-y-2">
          <div className="flex items-center gap-2">
            <MapPin className="h-5 w-5 text-primary" />
            <h1 className="text-xl md:text-2xl font-bold">Establishments Map</h1>
          </div>
          <p className="text-sm text-muted-foreground">
            View establishments by certification or registration status. Click a marker for details.
          </p>
        </div>
